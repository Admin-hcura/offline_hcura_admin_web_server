...
class Mail{
  ...
  async sendTempAppointmentBookedEmailToAdmin(booked, docDetails) {
    try {
      ...
      info.sendMail({
        ...
        html: (
          await emailTemplates.tempAppointmentBookedEmailToAdmin(booked, docDetails)).toString(),
      })
      .then(() => {
  ...
  async sendAppointmentConformedEmailToPT(details) {
    try {
      ...
      info.sendMail({
        ...
        subject: "Your appointment has been scheduled !",
        html: (
          await emailTemplates.appointmentConformedEmailToPT(details)).toString(),
      })
      .then(() => {
  ...
  async sendApptFormPtDetailsToAdmin(name, age, phoneNo,
    whatsAppNo, emailId, gender, state, consultationType, 
        ...
        info.sendMail({
          ...
          html: (
            await emailTemplates.sendApptFormPtDetailsRequest(
              name, age, phoneNo, whatsAppNo, emailId, gender, 
              state, consultationType, message, branch, formId,
              concern )).toString(),
        })
        .then(() => {
          console.log("Email sent");
  ...
  async sendContactUsInfoToAdmin(name, emailId, phoneNo, city, comment, contactId) {
    try {
      let info = await this.setUpSmtp();
      info
        .sendMail({
          ...
          html: (
            await emailTemplates.sendContactUsInfoRequest(name, emailId, phoneNo, city,
              comment, contactId)).toString(),})
        .then(() => {
          console.log("Email sent");
        })
        .catch((error) => {
  ...
  async sendCorporateInfoToAdmin(name, workEmail, phoneNo, companyName, companySize, 
  ...
  async sendOfferFormPtDetailsToAdmin(name, emailId, phoneNo, state, couponCode, offerId) {
    try {
      let info = await this.setUpSmtp();
      info
        .sendMail({
