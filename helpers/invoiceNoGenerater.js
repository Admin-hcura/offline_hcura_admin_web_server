  ...
  async generateInvoiceNumber(branchcode) {
    try {
      ...
      if (oldInvoice) {
        ...
        return `${branchcode}/01/${moment().format("DD")}/${moment().format("MMM")}/${moment().format("YYYY")}-SAC999319`;
      }
    } catch (e) {
      throw e;
    }
  ...
  async generateInvoiceNumberAsthetic(branchcode) {
    try {
      ...
      if (oldInvoice) {
        ...
        if (newNo > 9) {
          return `${branchcode}/${newNo}/${moment().format("DD")}/${moment().format("MMM")}/${moment().format("YYYY")}-SAC999729`;
        } else {
          return `${branchcode}/0${newNo}/${moment().format("DD")}/${moment().format("MMM")}/${moment().format("YYYY")}-SAC999729`;
        }
      } else {
        return `${branchcode}/01/${moment().format("DD")}/${moment().format("MMM")}/${moment().format("YYYY")}-SAC999729`;
      }
    } catch (e) {
      throw e;
    }
